...
#10、偏差 （Deviation）- 发散型条形图 （Diverging Bars）
# Prepare Data
df = pd.read_csv("https://github.com/selva86/datasets/raw/master/mtcars.csv")
x = df.loc[:, ['mpg']]
...
# Prepare Data发散型文本 （Diverging Texts）
df = pd.read_csv("https://github.com/selva86/datasets/raw/master/mtcars.csv")
x = df.loc[:, ['mpg']]
df['mpg_z'] = (x - x.mean())/x.std()
df['colors'] = ['red' if x < 0 else 'green' for x in df['mpg_z']]
...
# Prepare Data  12、发散型包点图 （Diverging Dot Plot）
df = pd.read_csv("https://github.com/selva86/datasets/raw/master/mtcars.csv")
x = df.loc[:, ['mpg']]
df['mpg_z'] = (x - x.mean())/x.std()
...
# Prepare Data  13 带标记的发散型棒棒糖图 （Diverging Lollipop Chart with Markers）
df = pd.read_csv("https://github.com/selva86/datasets/raw/master/mtcars.csv")
x = df.loc[:, ['mpg']]
df['mpg_z'] = (x - x.mean())/x.std()
df['colors'] = 'black'
...
#三、排序 （Ranking）15 有序条形图 （Ordered Bar Chart）
# Prepare Data
...
# Prepare Data16 棒棒糖图 （Lollipop Chart）
df_raw = pd.read_csv("https://github.com/selva86/datasets/raw/master/mpg_ggplot2.csv")
df = df_raw[['cty', 'manufacturer']].groupby('manufacturer').apply(lambda x: x.mean())
df.sort_values('cty', inplace=True)
df.reset_index(inplace=True)
...
# Import Data 18 坡度图 （Slope Chart）
...
#四、20、分布 （Distribution）-连续变量的直方图 （Histogram for Continuous Variable）
...
#五、组成 （Composition）-31 华夫饼图 （Waffle Chart）
...
#! pip install pywaffle,  31、组成 （Composition）-华夫饼图 （Waffle Chart）-1
from pywaffle import Waffle
...
#六、变化 （Change）35 时间序列图 （Time Series Plot）
...
#6 带波峰波谷标记的时序图 （Time Series with Peaks and Troughs Annotated）
...
##ACF在另一方面显示了任何给定滞后（时间序列）与当前序列的自相关，但是删除了滞后的贡献。
...
# 42-1  "Data Source: https://www.kaggle.com/olistbr/brazilian-ecommerce#olist_orders_dataset.csv"
from dateutil.parser import parse
from scipy.stats import sem
